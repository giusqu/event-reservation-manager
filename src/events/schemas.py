...
class EventResponse(CustomBase):
    id: PositiveInt
    name: str = Field(..., min_length=3, max_length=50, examples=["Sagra delle polpette"])
    date: datetime
...
class EventUpdate(CustomBase):
    name: Optional[str] = Field(None, min_length=3, max_length=50, examples=["Sagra delle polpette"])
